...
export default function CartScreen() {
  ...
  const handleCheckout = () => {
    ...
    const computedTotal = items.reduce((sum, it) => {
      const unit = typeof it.priceNumber === 'number' ? it.priceNumber : parsePrice(it.price, 0);
      return sum + unit * it.quantity;
    }, 0);

    Alert.alert('Proceed to Checkout', `Total: £${computedTotal.toFixed(2)}\n${itemCount} items`, [
      { text: 'Cancel', style: 'cancel' },
  ...
  // Ekranda daima doğru toplamı göstermek için güvenli toplam:
  const computedTotal = items.reduce((sum, it) => {
    const unit = typeof it.priceNumber === 'number' ? it.priceNumber : parsePrice(it.price, 0);
    return sum + unit * it.quantity;
  }, 0);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.cartItems} showsVerticalScrollIndicator={false}>
        <View style={styles.itemsContainer}>
          {items.map((item) => {
            const unitPrice =
              typeof item.priceNumber === 'number' ? item.priceNumber : parsePrice(item.price, 0);
      ...
      <View style={styles.checkoutContainer}>
        <TouchableOpacity style={styles.checkoutButton} onPress={handleCheckout}>
          <View style={styles.checkoutContent}>
            ...
            <Text style={styles.checkoutPrice}>£{computedTotal.toFixed(2)}</Text>
          </View>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  emptySubtitle: { fontSize: 16, color: '#666', textAlign: 'center', lineHeight: 24, marginBottom: 30 },
  shopButton: { backgroundColor: '#4CAF50', paddingHorizontal: 30, paddingVertical: 15, borderRadius: 25 },
  ...
  producerName: { fontSize: 12, color: '#4CAF50', fontWeight: '600', marginBottom: 4 },
  itemPrice: { fontSize: 14, color: '#666' },
  quantityControls: { flexDirection: 'column', alignItems: 'center', justifyContent: 'center', marginHorizontal: 15 },
  quantityButton: {
    width: 32, height: 32, borderRadius: 16, backgroundColor: '#f0f0f0',
    justifyContent: 'center', alignItems: 'center', marginVertical: 2,
  },
  quantity: { fontSize: 16, fontWeight: 'bold', color: '#333', marginVertical: 8, minWidth: 20, textAlign: 'center' },
  itemActions: { alignItems: 'flex-end', justifyContent: 'space-between' },
  itemTotal: { fontSize: 16, fontWeight: 'bold', color: '#4CAF50', marginBottom: 10 },
  removeButton: { padding: 5 },
  summaryContainer: {
    backgroundColor: '#fff', margin: 20, marginTop: 0, padding: 20, borderRadius: 12,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3,
  },
  ...
  summaryDivider: { height: 1, backgroundColor: '#e0e0e0', marginVertical: 10 },
  totalLabel: { fontSize: 18, fontWeight: 'bold', color: '#333' },
  totalValue: { fontSize: 18, fontWeight: 'bold', color: '#4CAF50' },
  checkoutContainer: { padding: 20, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#e0e0e0' },
  checkoutButton: { backgroundColor: '#4CAF50', borderRadius: 12, padding: 18 },
  checkoutContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  checkoutInfo: { flex: 1 },
  checkoutText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  checkoutSubtext: { color: '#fff', fontSize: 14, opacity: 0.9, marginTop: 2 },
